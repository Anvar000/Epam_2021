...
def wins_combinations(n: int = 1):
    """Generate wins combinations from board with size nxn

    Example of the board and wins combiations:
    ((a, b, c),
     (d, e, f),
     (g, h, i)) = (
        ((0, 0), (0, 1), (0, 2)),
    ...
    for i in range(n):
        line_horizontal = [(i, j) for j in range(n)]
        line_vertical = [(j, i) for j in range(n)]
